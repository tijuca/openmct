...
int read_serial(FILE *usbdevice, int g_bus, int g_dev) {
	...
	char *serial=NULL,
	     *vendorname=NULL,
		 *vendortype=NULL;		// char pointer for the splitted infos
	char *zdebug_buf;
	...
	debug_info(debug_buf);
		...
		while ((fgets (buf, sizeof (buf), usbdevice) != NULL) && (found <4)){
			//snprintf(debug_buf,99,"[%s/%d] while loop reached,\nbuf=\"%s\"\n",__FUNCTION__,__LINE__,buf);
			//debug_info(debug_buf);
			if(found <= 4){
				if(buf[0] == 'T'){
...
unsigned char *remove_spaces_end(char *input){
	...
	int r_length=0, z=0;
	//snprintf(debug_buf,99,"[%s/%d] help =  \"%s\"\n",__FUNCTION__,__LINE__,help);
	//debug_info(debug_buf);
	...
	if ((input[r_length-1]<=0x21) ){
		...
		for (z=r_length-1;  z>= 0; z--){
			...
			//snprintf(debug_buf,99,"[%s] current loop/character: %d/%x spaces = %d\n",__FUNCTION__,z,input[z],spaces);
			//debug_info(debug_buf);
		}
...
unsigned char *replace_spaces(char *vendoname, char *vendortype) {
	
	unsigned static char out[80];
	char help[80]="\0";
	int loop;
	
	strcpy(help,vendoname);
	strcat(help,"_");
	strcat(help,vendortype);
	for (loop=0;loop<=sizeof(help);loop++){
		if (help[loop] == ' ')
			help[loop] = '_';
	}
	strcpy(out,help);
