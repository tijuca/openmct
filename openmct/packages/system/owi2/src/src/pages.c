...
void glob_set(char *title) {
    ...
    if (strcmp(rc_start.nfs,"yes")==0) check="checked=\"checked\""; else check=" ";
    printf("        <input type=\"checkbox\" name=\"nfs\" value=\"yes\" %s>NFS\n",check);
    if (strcmp(rc_start.samba,"yes")==0) check="checked=\"checked\""; else check=" ";
    printf("        <input type=\"checkbox\" name=\"samba\" value=\"yes\" %s>Samba\n",check);
    if (strcmp(rc_start.ftpd,"yes")==0) check="checked=\"checked\""; else check=" ";
    printf("        <input type=\"checkbox\" name=\"ftp\" value=\"yes\" %s>FTP</p>\n",check);

    printf("        <h3 class=\"content_text\">Start/Stop Fernzugangs-Dienste:</h3>\n");
    printf("        <p class=\"content_text\">\n");
    if (strcmp(rc_start.sshd,"yes")==0) check="checked=\"checked\""; else check='\0';
    printf("        <input type=\"checkbox\" name=\"ssh\" value=\"yes\" %s>SSH\n",check);
    if (strcmp(rc_start.telnetd,"yes")==0) check="checked=\"checked\""; else check='\0';
    printf("        <input type=\"checkbox\" name=\"telnet\" value=\"yes\" %s>Telnet</p>\n",check);
    printf("        <h3 class=\"content_text\">Start/Stop System-Dienste:</h3>\n");
    printf("        <p class=\"content_text\">\n");
    if (strcmp(rc_start.portmap,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"portmap\"   value=\"yes\" %s>Portmap\n",check);
    if (strcmp(rc_start.syslogd,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"syslog\"    value=\"yes\" %s>Syslog\n",check);
    if (strcmp(rc_start.ntpdate,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"ntpdate\"   value=\"yes\" %s>NTPDate\n",check);
    if (strcmp(rc_start.crond,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"cron\"      value=\"yes\" %s>Cron\n",check);
    if (strcmp(rc_start.leds,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"led\"      value=\"yes\" %s>LEDs\n",check);
    if (strcmp(rc_start.hdparm,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"hdparm\"    value=\"yes\" %s>HDParm\n",check);
    if (strcmp(rc_start.swap,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"swap\"      value=\"yes\" %s>Swap\n",check);
    if (strcmp(rc_start.httpd,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"http\"      value=\"yes\" %s>HTTP<br>\n",check);
    if (strcmp(rc_start.ethtool_eth0,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"etool_eth0\" value=\"yes\"%s >ETHTool f&#252r ETH0 <small>(WAN Port)</small>\n",check);
    if (strcmp(rc_start.ethtool_eth1,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"etool_eth1\" value=\"yes\"%s >ETHTool f&#252r ETH1 <small>(Switch Ports)</small>\n",check);
    if (strcmp(rc_start.smartd,"yes")==0) check="checked=\"checked\""; else check=" ";;
    printf("        <input type=\"checkbox\" name=\"smartd\"      value=\"yes\" %s>Smartd\n",check);
    if (strcmp(rc_start.vservers,"yes")==0) check="checked=\"checked\""; else check=" ";;
    ...
    if (strcmp(options.led_in_device,"eth0")==0) check="selected"; else check=" ";
    printf("                <option value=\"eth0\" %s>eth0 (WAN Port)</option>\n",check);
    if (strcmp(options.led_in_device,"eth1")==0) check="selected"; else check=" ";
    ...
    if (strcmp(options.led_in_direction,"r")==0) check="selected"; else check=" ";
    printf("                <option value=\"r\" %s>eingehend (receive)</option>\n",check);
    if (strcmp(options.led_in_direction,"t")==0) check="selected"; else check=" ";
    ...
    if (strcmp(options.led_out_device,"eth0")==0) check="selected"; else check=" ";
    printf("                <option value=\"eth0\" %s>eth0 (WAN Port)</option>\n",check);
    if (strcmp(options.led_out_device,"eth1")==0) check="selected"; else check=" ";
    printf("                <option value=\"eth1\" %s>eth1 (Switch Ports)</option>\n",check);
    printf("            </select></td>\n");
    printf("        </tr\n");
    ...
    if (strcmp(options.led_out_direction,"r")==0) check="selected"; else check=" ";
    printf("                <option value=\"r\" %s>eingehend (receive)</option>\n",check);
    if (strcmp(options.led_out_direction,"t")==0) check="selected"; else check=" ";
    printf("                <option value=\"t\" %s>ausgehend (transmit)</option>\n",check);
    printf("            </select></p></td>\n");
    printf("        </tr\n");
    ...
    if (strcmp(options.ethtool_eth0_dm,"half")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth0_duplex\" value=\"half\" %s>HalbDuplex</td>\n",check);
    if (strcmp(options.ethtool_eth0_dm,"full")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth0_duplex\" value=\"full\" %s>FullDuplex</td>\n",check);
    printf("        </tr>\n");
    printf("        <tr>\n");
    printf("            <td>Autonegotion</td>\n");
    if (strcmp(options.ethtool_eth0_nego,"on")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth0_nego\" value=\"on\" %s>an</td>\n",check);
    if (strcmp(options.ethtool_eth0_nego,"off")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth0_nego\" value=\"off\" %s>aus</td>\n",check);
    printf("        </tr>\n");
    printf("        <tr>\n");
    printf("            <td>Geschwindigkeit</td>\n");
    if (options.ethtool_eth0_speed==10) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth0_speed\" value=\"10\" %s> 10MBit</td>\n",check);
    if (options.ethtool_eth0_speed==100) check="checked=\"checked\""; else check=" ";
...
#warning "The Wake On LAN can more modi then the default 'bg' !"
    ...
    printf("        <tr>\n");
    printf("            <td>Duplexmodus</td>\n");
    if (strcmp(options.ethtool_eth1_dm,"half")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth1_duplex\" value=\"half\" %s>HalbDuplex</td>\n",check);
    if (strcmp(options.ethtool_eth1_dm,"full")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth1_duplex\" value=\"full\" %s>FullDuplex</td>\n",check);
    printf("        </tr>\n");
    printf("        <tr>\n");
    printf("            <td>Autonegotion</td>\n");
    if (strcmp(options.ethtool_eth1_nego,"on")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth1_nego\" value=\"on\" %s>an</td>\n",check);
    if (strcmp(options.ethtool_eth1_nego,"off")==0) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth1_nego\" value=\"off\" %s>aus</td>\n",check);
    printf("        </tr>\n");
    printf("        <tr>\n");
    printf("        <td>Geschwindigkeit</td>\n");
    if (options.ethtool_eth1_speed==10) check="checked=\"checked\""; else check=" ";
    printf("            <td><input type=\"radio\" name=\"eth1_speed\" value=\"10\" %s> 10MBit</td>\n",check);
    if (options.ethtool_eth1_speed==100) check="checked=\"checked\""; else check=" ";
    ...
    if (strcmp(options.ntpip,"193.49.205.17")==0) check="selected"; else check=" ";
    ...
    if (strcmp(options.ip_forwarding,"yes")==0) check="checked=\"checked\""; else check=" ";
...
#warning "No checkbox for swap!"
    ...
    if (options.hd_transfer_mode==100) check="checked=\"checked\""; else check=" ";
    printf("            UDMA Modus 4<input type=\"radio\" name=\"udma\" value=\"100\" %s><br>\n",check);

    printf("            <p class=\"content_text\">Demaskierung anderer IRQs\n");
    if (options.hd_interrupt_umask==1) check="checked=\"checked\""; else check=" ";
    ...
    if (options.hd_sector_count==2) check="selected"; else check=" ";
    printf("                <option value=\"2\" %s>2</option>\n",check);
    if (options.hd_sector_count==4) check="selected"; else check=" ";
    printf("                <option value=\"4\" %s>4</option>\n",check);
    if (options.hd_sector_count==8) check="selected"; else check=" ";
    printf("                <option value=\"8\" %s>8</option>\n",check);
    if (options.hd_sector_count==16) check="selected"; else check=" ";
    ...
    if (options.hd_spindown==12) check="checked=\"checked\""; else check=" ";
    printf("            1min <input type=\"radio\" name=\"hd_to\" value=\"12\" %s>\n",check);
    if (options.hd_spindown==36) check="checked=\"checked\""; else check=" ";
    printf("            3min <input type=\"radio\" name=\"hd_to\" value=\"36\" %s>\n",check);
    if (options.hd_spindown==60) check="checked=\"checked\""; else check=" ";
    printf("            5min <input type=\"radio\" name=\"hd_to\" value=\"60\" %s>\n",check);
    if (options.hd_spindown==240) check="checked=\"checked\""; else check=" ";
    printf("            30min <input type=\"radio\" name=\"hd_to\" value=\"240\" %s>\n",check);
    if (options.hd_spindown==241) check="checked=\"checked\""; else check=" ";
    printf("            60min <input type=\"radio\" name=\"hd_to\" value=\"241\" %s>\n",check);
    if (options.hd_spindown==0) check="checked=\"checked\""; else check=" ";
